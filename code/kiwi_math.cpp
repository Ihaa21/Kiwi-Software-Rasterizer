...
{
    b32 Result = Num / Denom - (u32)(((f32)Num/(f32)Denom) + 0.5f) == 0;
    return Result;
}

inline i32 RoundUp(f32 Val)
{
    i32 Result = (i32)ceil(Val);
    return Result;
}

inline i32 RoundDown(f32 Val)
{
    i32 Result = (i32)floor(Val);
    return Result;
}
...
inline m4 RotationMat(f32 AngleX, f32 AngleY, f32 AngleZ)
{
    m4 Result = IdentityM4();
...
inline m4 TranslationMat(f32 X, f32 Y, f32 Z)
{    
    m4 Result = IdentityM4();
    Result.v[3].x = X;
    Result.v[3].y = Y;
